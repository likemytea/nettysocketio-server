...
public class ConsistentHashLoadBalance {
	...
	/**
	 * 初始化哈希环 循环计算每个node名称的哈希值，将其放入treeMap
	 */
	...
	private void printTreeNode() {
		if (virtualNodes != null && !virtualNodes.isEmpty()) {
			virtualNodes.forEach(
					(hashKey, node) -> System.out.println(new StringBuffer(node).append(" ==> ").append(hashKey)));
		} else
	...
	public static void main(String[] args) {
		...
		nodes.add("172.16.14.103:9099");
		nodes.add("172.16.176.51:9099");
		...
		System.out.println(consistentHash.hash("1", 0) + "====:" + consistentHash.selectNode("1"));
	}
}